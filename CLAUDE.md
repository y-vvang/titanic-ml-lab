...
- **Serverless 不能运行时 pip install**——Python 依赖在根目录 `requirements.txt` 声明，由 Vercel Python builder 构建时安装（版本与本地对齐并固定，`random_state=42` 的可复现性依赖库版本）。
...
bash ./scripts/prepare.sh     # 首次运行前：pip install -r requirements.txt
pnpm dev                      # 开发服务器（next dev，端口 5000）
pnpm build                    # pnpm install → pip install（Vercel 上跳过，见上）→ next build
pnpm start                    # 生产模式启动（next start，端口 5000）
pnpm lint                     # ESLint
pnpm ts-check                 # TypeScript 检查
pnpm validate                 # 顺序跑 ts-check + lint:build（Windows cmd 下 --parallel 正则会坏，别改回并行）
```

- 开发环境需先跑 `prepare.sh` 装 Python 依赖，否则 API 路由全部 500。Python 依赖的权威清单是根目录 `requirements.txt`（与 Vercel 构建共用；`prepare.sh`/`build.sh` 均直接 `pip install -r requirements.txt`，改依赖只动这一个文件）。
...
- `src/lib/i18n/`：`en.ts` 是事实来源，`zh.ts` 以 `typeof en` 约束（键漂移编译报错），组件经 `useI18n()` 取 `t('dotted.key')`；动态键（如 `` t(`versions.modelType.${type}`) ``）也走字典，缺失回退 en → 键名本身。
- 语言由 `navigator.language` 自动检测（zh* → 中文，其余 → 英文），无切换 UI、无持久化、无路由。`app/layout.tsx` `<body>` 顶部的内联阻塞脚本在首屏前写 `document.documentElement.dataset.lang`；`I18nProvider` 用 `useSyncExternalStore` 在 hydration 提交时立即应用（服务端快照恒为 en，保证不产生 hydration mismatch，也无需把路由动态化——若改用 cookies() 读取会强制动态渲染，已明确放弃）。
- Python 引擎输出双语：`api/py/shared/ml_engine.py` 的 STRINGS 表 + `_t()`，由 API 路由把 lang 传给引擎。
