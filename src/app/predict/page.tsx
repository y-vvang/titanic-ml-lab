...
export default function PredictPage() {
  ...
  return (
    <div className="space-y-8 p-5">
      <div>
        <h1 className="text-3xl font-bold text-foreground">自定义预测</h1>
        <p className="text-muted-foreground mt-2">输入乘客信息，看看模型预测的生还概率</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          {isLogisticWithMissing && (
            <div className="glass-card p-4 border border-destructive/30">
              <p className="text-sm text-destructive">当前模型为逻辑回归且存在未填充的缺失值，CabinDeck 和 Title 输入已禁用</p>
            </div>
          )}
          ...
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold text-foreground mb-4">乘客信息</h2>
            <div className="space-y-5">
              {/* Pclass */}
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">船舱等级 (Pclass)</label>
                <select value={input.Pclass}
                  ...
                  <option value={1}>1 等舱 - 上层</option>
                  <option value={2}>2 等舱 - 中层</option>
                  <option value={3}>3 等舱 - 下层</option>
                </select>
              </div>

              {/* Sex */}
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">性别 (Sex)</label>
              ...
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">
                  年龄 (Age): <span className="text-accent font-mono">{input.Age}</span> 岁
                </label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-foreground block mb-2">兄弟姐妹/配偶数</label>
                  <input type="number" value={input.SibSp} min={0} max={8}
                    onChange={e => setInput(p => ({ ...p, SibSp: parseInt(e.target.value) || 0 }))}
                    className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" />
                </div>
                <div>
                  <label className="text-sm font-medium text-foreground block mb-2">父母/子女数</label>
              ...
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">
                  票价 (Fare): <span className="text-accent font-mono">{input.Fare}</span> 英镑
                </label>
              ...
              {/* Embarked */}
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">登船港口 (Embarked)</label>
                <select value={input.Embarked}
                  onChange={e => setInput(p => ({ ...p, Embarked: e.target.value }))}
                  className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30">
                  <option value="S">S - 南安普顿</option>
                  <option value="C">C - 瑟堡</option>
                  <option value="Q">Q - 皇后镇</option>
                </select>
              </div>
              ...
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">舱面 (CabinDeck)</label>
                <select value={input.CabinDeck || ''}
                  ...
                  <option value="A">A - 甲板A</option>
                  <option value="B">B - 甲板B</option>
                  <option value="C">C - 甲板C</option>
                  <option value="D">D - 甲板D</option>
                  <option value="E">E - 甲板E</option>
                  <option value="F">F - 甲板F</option>
                  <option value="G">G - 甲板G</option>
                </select>
              </div>
              )}
              ...
              <div>
                <label className="text-sm font-medium text-foreground block mb-2">头衔 (Title)</label>
                <select value={input.Title || ''}
                  onChange={e => setInput(p => ({ ...p, Title: e.target.value }))}
                  disabled={isLogisticWithMissing}
                  className="w-full bg-muted border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 disabled:opacity-50 disabled:cursor-not-allowed">
                  <option value="Mr">Mr - 先生</option>
                  <option value="Mrs">Mrs - 女士（已婚）</option>
                  <option value="Miss">Miss - 小姐（未婚）</option>
                  <option value="Master">Master - 少爷</option>
                  <option value="Rare">Rare - 稀有头衔</option>
                </select>
              </div>
              )}
          ...
          <div className="space-y-6">
            {/* Gauge */}
            <div className="glass-card p-6">
              <h2 className="text-lg font-semibold text-foreground mb-4">生还概率</h2>
                ...
                {result ? (
                  <div className="text-center mt-2">
                    ...
                    <p className="text-sm mt-1 font-medium" style={{ color: getProbColor(prob) }}>
                      生还概率{getProbLabel(prob)}
                    </p>
                  </div>
                ) : (
                  <p className="text-muted-foreground text-sm mt-4">填写信息后点击预测</p>
                )}
              </div>
            </div>
            ...
            {result && (
              <div className="glass-card p-6">
                <h2 className="text-lg font-semibold text-foreground mb-3">解读</h2>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {prob >= 0.7
                    ? '基于当前模型，此类乘客的生还概率较高。女性和高等舱位乘客有更高的生还机会。'
                    : prob >= 0.4
                    ? '基于当前模型，此类乘客的生还概率处于中等水平。多个因素的综合影响决定了结果。'
                    : '基于当前模型，此类乘客的生还概率较低。男性、低等舱位和较高年龄降低了生还机会。'}
                </p>
                {/* Top Factors */}
                {result.topImpacts && result.topImpacts.length > 0 && (
                  <div className="mt-4">
                    <p className="text-xs text-muted-foreground mb-2">主要影响因素:</p>
      ...
      <div className="flex justify-between items-center">
        ...
        <p className="text-xs text-muted-foreground">你可以修改参数重新预测，或返回调整模型</p>
      </div>
    </div>
  );
}
